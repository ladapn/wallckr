...
void test_p_regulator_within_insensitivity_band_is_zero(void) {
    ...
    TEST_ASSERT_EQUAL(0, regulator.action(SETPOINT_CM - 1)); // e = 1, within insensitivity
}
...
void test_p_regulator_clamps_to_upper_limit(void) {
    const int GAIN = 10;
    const int ACTION_LIMIT_UP = 45; // mirrors Regulator's internal m_action_limit_up
    Regulator_P<int> regulator(GAIN, SETPOINT_CM, INSENSITIVITY_CM);

    TEST_ASSERT_EQUAL(ACTION_LIMIT_UP, regulator.action(0)); // e = 25, raw out = 250
}

void test_p_regulator_clamps_to_lower_limit(void) {
    const int GAIN = 10;
    const int ACTION_LIMIT_BOTTOM = -30; // mirrors Regulator's internal m_action_limit_bottom
    Regulator_P<int> regulator(GAIN, SETPOINT_CM, INSENSITIVITY_CM);

    TEST_ASSERT_EQUAL(ACTION_LIMIT_BOTTOM, regulator.action(50)); // e = -25, raw out = -250
}
...
void test_pd_regulator_adds_derivative_term_on_subsequent_action(void) {
    ...
    regulator.action(SETPOINT_CM - FIRST_ERROR_CM); // primes derivative state (last_e = FIRST_ERROR_CM)
    TEST_ASSERT_EQUAL(GAIN * SECOND_ERROR_CM + (SECOND_ERROR_CM - FIRST_ERROR_CM) * D_GAIN,
...
void test_pd_regulator_keeps_last_error_across_insensitivity_band(void) {
    ...
    regulator.action(SETPOINT_CM - FIRST_ERROR_CM); // primes derivative state (last_e = FIRST_ERROR_CM)
    regulator.action(SETPOINT_CM); // e = 0, inside insensitivity band, derivative state left untouched
    // derivative term uses the stale last_e (FIRST_ERROR_CM) rather than the skipped e (0)
    TEST_ASSERT_EQUAL(GAIN * THIRD_ERROR_CM + (THIRD_ERROR_CM - FIRST_ERROR_CM) * D_GAIN,
